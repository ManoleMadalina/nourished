...
nutrition & wellness platform. Built as self-contained HTML files — no build
...
If images or fonts don't load, try `nourishED-prototype-standalone.html`
instead — it has assets inlined for fully offline use.
...
- `nourishED-prototype.html` — the prototype (references external assets/fonts).
- `nourishED-prototype-standalone.html` — same prototype with assets inlined for offline use.
- `nourishED-export/` — exported copy of the prototype plus its own README.
- `uploads/` — pasted images and an additional copy used during prototyping.

---

## Notes for handoff

- **Single file.** All markup, styles, and logic live in each prototype HTML
  file. State is held in a plain JavaScript object and the UI re-renders on
  change — no framework required.
